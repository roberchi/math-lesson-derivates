    ...
    <>
      <Paper elevation={0} sx={{ p: 2.25, border: '1px solid', borderColor: 'rgba(184,138,29,.35)', bgcolor: 'custom.goldLight' }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ sm: 'center' }} gap={2}>
          ...
          <Box sx={{ flex: 1 }}><Typography variant="caption" sx={{ color: 'custom.gold', fontWeight: 800 }}>UNA STORIA DI IDEE</Typography><Typography fontWeight={750}>{summary}</Typography></Box>
          <Button color="warning" onClick={() => setOpen(true)}>Approfondisci</Button>
        </Stack>
      </Paper>
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ justifyContent: 'space-between', px: 3 }}>
          {href ? <Link href={href} target="_blank" rel="noreferrer" display="inline-flex" alignItems="center" gap={.5}>Fonte esterna <ArrowOutwardRoundedIcon fontSize="small" /></Link> : <span />}

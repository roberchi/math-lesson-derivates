...
import { coreSections, enrichmentSections, lessonOneSections, lessonTwoSections, type CourseSection } from '@/data/course';
...
function SidebarContent({ close }: { close?: () => void }) {
  ...
  const enrichmentRead = enrichmentSections.filter((section) => completed.includes(section.id)).length;
  const percent = coreRead / coreSections.length * 100;
...
function NavItem({ title, icon, active, onClick }: { title: string; icon: ReactNode; active: boolean; onClick: () => void }) {
  return (
    <ListItemButton selected={active} onClick={onClick} sx={{ mb: .25, borderRadius: 1.25, minHeight: 42, color: 'inherit', '&.Mui-selected': { bgcolor: 'rgba(127,148,244,.17)' }, '&:hover': { bgcolor: 'rgba(255,255,255,.06)' } }}>
      ...
      <ListItemText primary={<Typography sx={{ fontSize: '.78rem', lineHeight: 1.25, whiteSpace: 'normal', fontWeight: active ? 700 : 500 }}>{title}</Typography>} />
    </ListItemButton>
  );
}
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const globalProgress = coreSections.filter((section) => completed.includes(section.id)).length / coreSections.length * 100;
